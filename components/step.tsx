...
const Step: NextPage<StepType> = ({ className = "" }) => {
  return (
    <div
      className={`w-[76.25rem] h-[34.375rem] rounded-[28px] bg-[#181f25] overflow-hidden shrink-0 flex items-start p-[4.25rem] box-border gap-[3rem] max-w-full z-[1] mq800:gap-[1.5rem] mq1350:h-auto mq1350:flex-wrap mq1350:pl-[2.125rem] mq1350:pr-[2.125rem] mq1350:box-border ${className}`}
    >
      <section className="flex-1 flex flex-col items-start gap-[1.25rem] text-center text-[1.25rem] text-[#0bc111] font-['Proxima_Nova'] mq800:min-w-full">
      ...
      <Image
        className="w-[27.125rem] relative rounded-2xl max-h-full object-cover max-w-full mq1350:flex-1"
        loading="lazy"
        width={434}
        height={280}
        sizes="100vw"
